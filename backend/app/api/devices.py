...
async def check_device_heartbeat(
    ...
    if is_active is not None:
        query = query.filter(DeviceAuthentication.is_active == is_active)
    ...
    return [{
        ...
        "last_authenticated": d.last_authenticated.isoformat() if d.last_authenticated else None,
...
async def register_device(
    ...
    existing = db.query(DeviceAuthentication).filter(
        DeviceAuthentication.sensor_id == request.sensor_id
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="Device already registered for this sensor")

    api_key = secrets.token_urlsafe(32)
    mqtt_password = secrets.token_urlsafe(16)
    mqtt_password_hash = hashlib.sha256(mqtt_password.encode()).hexdigest()

    device_auth = DeviceAuthentication(
        sensor_id=request.sensor_id,
        device_id=sensor.device_id,
        api_key=api_key,
        mqtt_username=request.mqtt_username or sensor.device_id,
    ...
    return {
        ...
        "api_key": api_key,
        "mqtt_username": device_auth.mqtt_username,
...
async def update_device(
    ...
    device = db.query(DeviceAuthentication).filter(DeviceAuthentication.id == device_id).first()
    if not device:
        raise HTTPException(status_code=404, detail="Device not found")
    _ensure_device_access(device, db, current_user)

    if request.is_active is not None:
        device.is_active = request.is_active
    if request.expires_at is not None:
        device.expires_at = datetime.fromisoformat(request.expires_at)

    device.updated_at = datetime.utcnow()
    db.commit()

    return {"message": "Device updated", "is_active": device.is_active}
...
async def rotate_api_key(
    ...
    return {
        "api_key": new_api_key,
        "message": "API key rotated - store it securely"
    }
